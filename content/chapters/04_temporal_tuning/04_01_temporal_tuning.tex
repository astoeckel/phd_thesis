...
Some neurons in the brain exhibit pronounced temporal activity patterns \emph{in situ}.
For example, as is illustrated in \Cref{fig:temporal_responses_examples}, their activities may extend beyond the stimulus period, cease before the stimulus has ended, repeat rhythmically, or be tuned to specific spatio-temporal patterns---or likely any combination of the above.
Theoretical work (that we review below) suggests that diverse temporal responses form a representation of the past stimulus history \citep{howard2014unified,voelker2018improving} and could, for example, support delay learning in the cerebellum (\cite{fujita1982adaptive,medina2000computer}; see also \Cref{chp:cerebellum}).
%Furthermore, temporal response patterns play an important role in time-perception \citep{buhusi2005what,allman2014properties}.
While spatio-temporal neural responses have originally been extensively studied in visual cortex \citep[e.g.,][]{deangelis1993spatiotemporal,carandini1999linearity}, more recent evidence points at various brain areas---including hippocampus, striatum, and cerebellum---possessing neurons with pronounced temporal responses \citep{lusk2016cerebellar}.
...
As illustrated in \Cref{fig:temporal_responses_examples_c}, time cells reach their peak activity a certain time $\theta$ after the stimulus onset at $t_0$.%
...
This delay $\theta$ is specific to each cell and can be in the milliseconds to seconds range.
Populations of time cells encode $t - t_0$, or, assuming continuous input signals, represent a compressed version of the stimulus history.
...
\end{align*}
This model is biologically plausible, reproduces empirical data, and has been proposed as a model for spatial representations in hippocampus as well.
Still, as we discuss below, there are theoretical reasons why relying on sums of exponential filters can be suboptimal.
\Citet{voelker2018improving} demonstrate that the recurrent Legendre Delay Network (LDN) similarly reproduces time cell data, while at the same time being mathematically optimal.
...
The basic observation is that some cells in visual cortex reach their maximum firing rate for specific stimulus sequences.
...
One possible way to construct a model of this spatio-temporal tuning is to use a \emph{space-time receptive field} $\mathfrak{e}(t; \xi_1, \xi_2)$, where $\xi_1$ and $\xi_2$ are coordinates in the visual field and $t \geq 0$ is time (\cite{watson1983look,adelson1985spatiotemporal}; see \cite{carandini1999linearity}, for a review).
...
The above summary of biological temporal response patterns has hopefully provided some motivation as for why modelling temporal tuning can be useful.
...
\begin{definition}
	...
	\begin{align}
		a_i(\vec{\mathfrak{x}})
			= G\left[ J_i \left( \int_{0}^\infty \!\!\! \big\langle \vec{\mathfrak{e}}_i(\tau), \vec{\mathfrak{x}}(-\tau) \big\rangle 	\,\mathit{d\tau} \right) \right]
		= G\left[ \alpha_i \! \int_{0}^\infty \!\!\! \big\langle \vec{\mathfrak{e}}_i(\tau), \vec{\mathfrak{x}}(-\tau) \big\rangle \,\mathit{d\tau} + \beta_i \right] \,,
...
This effectively unifies the NEF dynamics and representation principles; the weight solver can draw upon the dynamics of the pre-population as well as temporal resources such as synaptic filters to approximate the desired dynamics.
...
\footnote{Both the signal $\vec{\mathfrak{x}}$ and the encoder $\vec{\mathfrak{e}}$ must be normalised for the convolution between the encoder and the stimulus to correspond to the cosine similarity.
...
Let $a_j(\mathfrak{x}, t)$ be the activity of the $j$th pre-neuron at time $t \leq 0$, $J_i$ the current-translation function, $h_{ij}$ the synaptic filter for the connection onto the $i$th post-neuron, and let $\mathfrak{x}_k : \mathbb{R}^- \longrightarrow \mathbb{X}$ be one of $N$ random input sequences with $\lim_{t \to -\infty} \mathfrak{x}_k(t) = 0$ and finite norm $\|\mathfrak{x}_k\|$.%
...
Instead, the primary challenge when solving this optimisation problem lies in selecting suitable input sequences $\mathfrak{x}_k$ \citep[cf.][Section~10.2.5]{verhaegen2007filtering}.
...
LTI systems are quite useful, and we can always use the NEF representation principle (\Cref{sec:nef_representation}) to construct populations that represent $d$ dimensional quantities while using identity decoders to linearise the neural responses.
...
Phrasing the first limitation more precisely, maintaining uniform dynamics across a series of chained populations becomes progressively more difficult as the length of the chain increases.
...
Notably, this assumption effectively turns the system into a feed-forward network, thus greatly simplifying solving for recurrent weights (cf.~\Cref{fig:recurrent_temporal_tuning}).
...
\begin{restatable}{theorem}{thmTemporalLstsq}
	...
	\begin{align*}
		...
		& h(t) &= \tau^{-1} e^{-t \tau^{-1}} \,.
	\end{align*}
	Let $\mathfrak{x}_k : \mathbb{R}^- \to \mathbb{R}$ be one of $N \geq n + 1$ arbitrary (with some mild constraints; see proof) input sequences.
...
We provide a proof of this in \Cref{app:lstsq_nef_equivalence}.
...
\begin{figure}[p]
	...
	\caption[Realising LTI systems using temporal tuning curves]{
		...
		We use $N = 1000$ input sequences $\mathfrak{x}_k$ of length $T = \SI{10}{\second}$ sampled at $\Delta t = \SI{1}{\milli\second}$ consisting of white noise filtered with a second-order low-pass filter (time constant $\vartheta = \SI{100}{\milli\second}$).
...
Second, and most importantly, the strength of our approach lies in being agnostic about the network architecture; we can approximate connection weights for networks with higher-order and heterogeneous synaptic filters in the feedback and input paths.
...
\begin{figure}
	...
	Same training procedure as in \Cref{fig:recurrence_examples_1}; training sequences $\mathfrak{x}_k$ are white noise filtered with a second-order low-pass filter with time constant \SI{100}{\milli\second}.
...
\begin{figure}
	...
	\caption[Effect of the input sequence distribution the system error]{Effect of the input sequence distribution the system error.
	Same setup and legend as in \Cref{fig:heterogeneous_recurrence_exploration}, but for a recurrent network with three filters and with input sequences $\mathfrak{x}_k$ of varying bandwidth.
	Inputs are generated by low-pass filtering white noise with a second-order low-pass; the time-constant $\vartheta$ of that filter corresponds to the different line colours (dark blue corresponds high-bandwidth signals, orange to low-frequency signals).
...
Moreover, the ability of the network to generalise depends on the input sequences $\mathfrak{x}_k$ used for training.
...
We explore the effect of the training sequences on the final network in \Cref{fig:heterogeneous_recurrence_exploration_xs_flt}.
Specifically, we generate $\mathfrak{x}_k$ with different frequency content by varying the time-constant $\vartheta$ of the second-order low-pass filter that is applied to the generated white noise.
Unsurprisingly, only supplying low-frequency inputs (i.e., using a large $\vartheta$) results in networks that achieve low errors for low-frequency inputs.
A more wide-band input (i.e., using a small $\vartheta$) results in a compromise-solution with smaller errors for wide-band inputs, but (at least in the case of the integrator) both slightly higher errors for low-frequency inputs.
...
That is, we can treat the desired dynamics $\mathfrak{e}_i(t)$ as if they were the output of a black-box system in response to our input sequences $\vec{\mathfrak{x}}_k(t)$.
...
A central challenge of these approaches is robustness to noise in the measurements, which---even in the case of zero-mean white noise---causes asymptotic biases in the resulting system \citep{sagara1989recursive}.
