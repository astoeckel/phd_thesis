...
Mathematically speaking, we can decode higher-order functions $f$ from a temporally tuned neuron population that not only depend on the current represented value $\vec x \in \mathbb{R}^d$, but on a \emph{signal} $\mathfrak{x}(t)$ (cf.~\Cref{tbl:spatiotemporal}).
