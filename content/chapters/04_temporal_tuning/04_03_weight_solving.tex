...
So far we focused on synaptic filters as the primary temporal resource in spiking neural networks.
...
Below, we discuss the abstract optimisation problem, point out potential solutions, and demonstrate that we can account for \ALIF dynamics.
...
We then solve for weights $w_{ij}$ that, assuming that the pre-neurons $j$ adhere to their normative tuning, decode a current $J_i(\mathfrak{x}_k)$ that evokes the desired rate.
The mapping between currents and rates is according to a \emph{static} response curve $G[J]$.
It holds $a_i(\mathfrak{x}_k) = G[J_i(\mathfrak{x}_k)]$.
...
Since our weight optimisation problem is in current space, we somehow need to map from the rate determined by the tuning curve $a_i(\mathfrak{x}_k)$ onto a current.
More precisely we need a function of the form $\Omega^{-1}[\mathfrak{a}] : (\mathbb{R}^- \longrightarrow \mathbb{R}^+) \longrightarrow \mathbb{R}$.%
...
Now, our optimisation problem in \cref{eqn:weight_optimise_currents_temporal} becomes
\begin{align}
	E = \sum\nolimits_{k = 1}^N \Bigl[
...
The \ALIF model accounts for firing-rate adaptation observed in biological neurons (cf.~\Cref{fig:izhikevich_whichmod_figure1f}).
This is accomplished by increasing an exponentially decaying adaptation variable $n(t)$ by a constant amount $n_0$ every time the neuron spikes \citep{treves1993meanfield}.
The specific \ALIF model discussed here applies this spike-triggered adaptation mechanism to a current-based \LIF neuron---the adaptation variable $n(t)$ is simply subtracted from the input current $J(t)$.
...
\begin{figure}
	...
	\caption[Accounting for ALIF dynamics]{
		...
		\emph{Left:} Recurrent network implementing a \SI{2}{\hertz} undampened oscillator with $100$ standard \LIF neurons (maximum rates at \SI{200}{\per\second}).
...
To account for the \ALIF dynamics in \cref{eqn:weight_optimise_currents_temporal_nonlin}, we merely need to decode the adaptation current from the pre-population.
