...
Note that we abbreviate the \glsdisp{dennonlin}{two-compartment LIF nonlinearity} with conductance-based synapses as \Hcond, and the standard current-based \LIF neuron as \Hcur.
...
We sample random 2D target-current functions $J_\rho$ over $(x_1, x_2) \in [-1, 1]^2$ on a $63 \times 63$ grid using the technique illustrated in \Cref{fig:2d_functions_overview}.
...
Evidently, networks capable of approximating \XOR-like functions are, using our terminology form \Cref{sec:nlif_theory}, more \enquote{powerful} than those limited to conductance-based shunting.
