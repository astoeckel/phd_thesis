...
That is, we can use temporal tuning curves to implement any dynamical system that could be implemented using the dynamics principle.
...
\begin{align*}
	(\vec x )_i = \int_0^\infty \!\!\! h(\tau) f'_i(\vec x, \vec{\hat u}) \,\mathrm{d}\tau \,.
\end{align*}
