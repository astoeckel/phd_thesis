...
For $\tau_1 = \tau_2$ this type of synaptic dynamics follows the so-called \emph{alpha function}.
...
For example, the synaptic strength can depend on the amount of neurotransmitter released in the pre-synapse, or the ion-channel density in the post-synapse \citep[Chapter~12]{kandel2012principles}.
...
\begin{align}
	g(t)
		&= (u \ast h)(t)
		 = \int_0^\infty u(t - t') h(t') \,\D t'
		 = \int_0^\infty h(t') \sum\nolimits_{i}  w_i \sum\nolimits_{j} \delta(t - t_{ij} - t') \,\D t' \,.
...
From this signal-processing perspective, the synaptic dynamics attenuate certain frequencies in the input spike-train; they act as a \emph{filter}.
...
The models discussed in the previous section describe individual neurons at a relatively low level of abstraction.
This incurs a high computational cost, especially when considering detailed compartmental neurons with Hodgkin-Huxley-like super-threshold dynamics.
...
\begin{figure}[t]
	...
	\caption[Equivalent circuit diagram and membrane potential trace of an LIF neuron]{Equivalent circuit diagram and membrane potential trace of an \LIF neuron. \textbf{(A)} The \LIF model consists of a capacitative membrane with a \enquote{leak} channel. A reset mechanism clamps \vMem to the reset potential \vReset. \textbf{(B)} Membrane potential trace (\emph{top}) for rectangle pulse current inputs (\emph{middle}).  Vertical dashed lines are spike events. The bottom graph depicts the remaining refractory period.}
	\label{fig:lif}
\end{figure}
...
Response curves and time-less \enquote{rate neurons} form the basis for \emph{artificial} neural networks commonly used in machine learning. We discuss this in more detail later in this thesis.
Furthermore, we revisit temporal properties of the response curve when we discuss temporal tuning curves in \Cref{chp:temporal_tuning}.
...
Instead, we have to rely on the numerical method suggested by \cref{eqn:response_curve}.
That is, we simulate the neural dynamics over a sufficiently long time-period and count the number of spikes $n_\mathrm{spikes}$.
...
As we discussed at the beginning of this chapter, it can be desirable to build models that reliably adhere to certain constraints---such as the aforementioned tuning properties.
...
In turn, these ideas motivate \emph{population codes}.
We heavily rely on these codes in the context of our modelling tool, the Neural Engineering Framework (cf.~\Cref{sec:nef}).
...
Although tuning curves and receptive fields are reconstructed within a network context, they only characterise individual neurons.
...
Within a single layer of the same column, neurons possess similar preferred orientations.
