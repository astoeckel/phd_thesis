...
XS_FILTERS = list(np.logspace(-2, 1, 7))
N_XS_FILTERS = len(XS_FILTERS)
...
def solve(flts_A, flts_B, flt_tar, xs_tau):
    # Solve for weights
...
def execute_single(idcs):
    i_target, i_net, i_filter, i_xs_flt, i_seed = idcs

    target = TARGETS[i_target]
    net = NETWORKS[i_net]
    n_flts = FILTERS[i_filter]
    xs_tau = XS_FILTERS[i_xs_flt]
    ...
    if net == "ref":
        ...
        A, B = solve(flts_A, flts_B, flt_tar, xs_tau)
    ...
    idcs = list(
        filter(
            ...
            itertools.product(range(N_TARGETS), range(N_NETWORKS),
                              range(N_FILTERS), range(N_XS_FILTERS),
                              range(N_REPEAT))))

    fn = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data',
                      "heterogeneous_recurrence_exploration_xs_flt.h5")
    with h5py.File(fn, 'w') as f:
        f.attrs["filters"] = json.dumps(FILTERS)
        f.attrs["xs_filters"] = json.dumps(XS_FILTERS)
