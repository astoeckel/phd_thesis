...
# Sweep over the regularisation keys
print("Running regularisation factor experiments...")
args = [(param, reg, repeat) for param in range(N_SOLVER_PARAMS_SWEEP)
        for reg in range(N_REGS) for repeat in range(N_REPEAT)]
random.shuffle(args)

with multiprocessing.Pool(N_CPUS) as pool:
    errs = np.zeros((N_SOLVER_PARAMS_SWEEP, N_REGS, N_REPEAT, 2))
    for ((i, j, k), e1,
